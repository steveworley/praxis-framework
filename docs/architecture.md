...
├── agents/                    # modular playbooks (the role's verbs)
│   ├── persona.md             # who the role is, voice, hard rules
│   ├── {agent-name}.md        # one file per repeatable behavior
│   └── proposed/              # drafts awaiting acceptance (agent writes here, never to ../)
├── lib/                       # declarative reference data
│   └── *.yaml                 # role-authored, agent-readable
├── memory/                    # observational notebook (agent writes)
...
├── escalations/               # structured asks (agent writes; operator triages)
...
Universal directories are fixed: `agents/`, `lib/`, `memory/`, `escalations/`. The work-product directory is role-named.

## What lives where

| Question | Where it goes |
|---|---|
| "Who am I?" | `agents/persona.md` |
| "How do I work?" | `CLAUDE.md` (top-level operating manual) |
| "What are my repeatable behaviors?" | `agents/{name}.md` |
...
| "I drafted a new behavior" | `agents/proposed/{name}.md` + an escalation referencing it |
...
Markdown with optional frontmatter (Phase 4 will introduce a verb taxonomy here):
...
You are {persona ref}. {What this agent does in one sentence.}

**Read `agents/persona.md` first.** {Any voice notes specific to this behavior.}
...
Frontmatter is optional today; Phase 4 makes it the basis for verb-grouping in the dashboard.
...
proposed_skill: <optional path to agents/proposed/{name}.md>
status: open
---
...
- **Identity preamble** — "You are {persona}; read `agents/persona.md` first"
- **Agents table** — what agents exist, when each runs, input → output
- **Pipeline / workflow** — how agents compose (if applicable)
...
- **Persona** — parsed from `agents/persona.md` (identity, voice, role)
...
- **Activity** — recent agent runs (parsed from logs)

Phase 1 dockerizes this. Phase 3 adds a role-planning UX. Phase 4 adds verb-grouped agent views. Phase 2 makes the dashboard the runtime UI.
