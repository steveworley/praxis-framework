...
The seed logic (`dashboard/src/lib/seed-role.ts`, `research-engine.ts`) extracts into a shared internal module both the CLI and the dashboard consume.

### Repo layout — monorepo with subdirs (for now)

The praxis-framework repo grows into a monorepo with subdirs per surface. No separate npm packages, no separate git repos — everything in this tree, single git history, single CI pipeline:

```
praxis-framework/
├── cli/                    # Phase 1.5: Node CLI tool (npx praxis init)
├── dashboard/              # existing: Astro + Node SSR
├── packages/
│   └── seed/              # Phase 1.5: shared seed logic, used by cli/ and dashboard/
├── runtime/                # Phase 2.0: durable runtime container
├── adapters/               # Phase 2.0: tool adapter implementations
│   ├── mcp-google-workspace/
│   ├── mcp-slack/
│   └── mcp-playwright/
├── template/               # existing: role scaffolding
├── docs/                   # existing
└── docker-compose.yml      # canonical deployment (Phase 1.5)
```

**Why monorepo for now:**

- **Lower friction for cross-package changes** — schema changes that touch the seed package, the CLI, AND the dashboard land in one PR. No version bumps, no lockstep release dance, no `npm publish` sequence.
- **Single git history** — every architectural decision is one commit graph. Reviewers see the full picture.
- **One CI pipeline** — tests run against the whole stack on every change.
- **Refactor freedom** — nothing's stable yet; locking interfaces between packages prematurely is a tax we don't need to pay.

**When we'd split:**

- One of the packages becomes useful outside praxis (the seed logic for community role-spec tooling, the runtime for non-praxis agentic workloads, etc.)
- Independent release cadences emerge (the runtime ships weekly, the dashboard ships monthly)
- A different team takes ownership of a sub-component

Splitting later is straightforward — `git subtree split --prefix=cli` extracts a subdirectory's history into a standalone repo when needed. We don't need to over-design for that case now.
...
1. **Extract seed logic** to `packages/seed/` (shared internal module). Currently lives in `dashboard/src/lib/seed-role.ts`. Make it runtime-agnostic so both the CLI and the (eventual) dashboard can consume it. Single workspace, no package publishing.
2. **CLI tool** at `cli/` (`npx praxis init` via the workspace bin) — interactive prompts mirroring the existing wizard, plus tool selection and trigger pre-fill. Output: configured role repo + `docker-compose.yml` + `.env.example`.
